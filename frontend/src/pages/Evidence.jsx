    ...
    <div className="page-stack">
      <section className="evidence-hero">
        <div>
          ...
          <p>
            Every captured field image can carry a content hash and canonical
            payload hash. The website surfaces the comparison without exposing
            private server internals.
          </p>
        </div>
        <div className="ledger-glyph">
          <div>SHA</div>
          <small>256</small>
        </div>
      </section>
      <section className="panel table-panel">
        <div className="panel-head">
          ...
          <span className="verified-badge">124 verified</span>
        </div>
        <div className="case-table evidence-table">
          ...
          {rows.map((r) => (
            <Link className="case-row" key={r[0]} to={`/cases/${r[0]}`}>
              <span className="case-id">{r[0]}</span>
              <code>{r[1]}</code>
              <code>{r[2]}</code>
              <span className={`integrity ${r[3]}`}>
                {r[3] === "verified" ? "✓ verified" : "△ review"}
              </span>
            </Link>
          ))}
        </div>
